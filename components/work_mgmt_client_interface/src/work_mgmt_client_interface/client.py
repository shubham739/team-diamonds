...
class IssueTrackerClient(ABC):
    ...
    def get_issues(
        ...
        *, # asterisk indicates that all calls to this method must specify the argument name: get_issues(title="Sample Title")
        ...
        Yields:
            An iterator of Issue instances matching all supplied filters. Iterator handles pagination of API results more seamlessly than returning all results at once
    ...
    def update_issue(self, issue_id: str, update: IssueUpdate) -> Issue:
        ...
        Notes on usage: Applies a set of changes to an existing issue and return the updated Issue. Only the fields explicitly set on will be modified, leaving the other properties unchanged
    ...
    def delete_issue(self, issue_id: str) -> None:
        ...
        Notes on usage:
            Deletes an issue by ID. Consideration will need to be made on implementation for delete, as some Issue Trackers do not allow deletion without archiving first, though this does not need to be handled here in the interface
...
def get_client(*, interactive: bool = False) -> IssueTrackerClient:
    """
    Args:
        interactive: When True, the implementation can pause, prompt the user for input (login credentials), and wait for input
                     When False, the implementation should rely solely on environment variables or pre-configured credentials - good for a development environment.
